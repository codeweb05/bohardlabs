...
| 4   | `@bohardlabs/api-client`   | `lib/axios`, `lib/auth`, `lib/react-query`, perms  |  1200 | 2-4  | [plan](../superpowers/plans/open/2026-08-28-api-client-package.md)   |
| 5   | `@bohardlabs/admin-shell`  | `components/layouts/` + sidebar context            |  1120 |  4   | [plan](../superpowers/plans/open/2026-08-28-admin-shell-package.md)  |

Order matters in one place only: `@bohardlabs/admin-ui` before `@bohardlabs/form`, because
`CancelButton` renders `UnsavedChangesDialog`. Everything else is independent.

### 1. `@bohardlabs/image-editor`
...
return later behind a `@bohardlabs/form/maps` subpath; `BuildingSelectField` is domain and never
will.

### 4. `@bohardlabs/api-client`
...
`@bohardlabs/admin-ui` supplies pieces it renders.
...
wanted at once, one `@bohardlabs/web-utils` is the shape to consider, not four packages.
...
- No `@/…` imports survive. Whatever an app supplied becomes a prop, an option or an
  adapter.
- No hardcoded user-facing string. The house pattern is the one `@bohardlabs/datatable` uses: a
...
- `"private": true` until the package is genuinely ready. See
  [decision 0008](../decisions/0008-npm-scope.md).
